...
def user_service(session: Annotated[AsyncSession, Depends(db_helper.get_async_session)]):
    return UserService(UserRepository(session))


def monster_service(session: Annotated[AsyncSession, Depends(db_helper.get_async_session)]):
    return MonsterService(MonsterRepository(session))


def player_service(session: Annotated[AsyncSession, Depends(db_helper.get_async_session)]):
    return PlayerService(PlayerRepository(session))


def encounter_service(session: Annotated[AsyncSession, Depends(db_helper.get_async_session)]):
